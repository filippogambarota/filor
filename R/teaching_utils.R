  ...
  if (!is.null(q)) {
    if (length(q) == 1) {
      if (lower.tail) {
        side <- -1
        pnorm_res <- sprintf(
          "pnorm(q = %s, mean = %s, sd = %s, lower.tail = TRUE) = %.3f",
          round(q, 2),
          mean,
          sd,
        ...
        side <- 1
        pnorm_res <- sprintf(
          "1 - pnorm(q = %s, mean = %s, sd = %s, lower.tail = TRUE) = %.3f",
          round(q, 2),
          mean,
          sd,
          pnorm(q, mean, sd, lower.tail = lower.tail)
      ...
      if (within) {
        ...
        pnorm_res <- sprintf(
          "pnorm(%s, %s, %s, lower.tail = TRUE) - pnorm(%s, %s, %s, lower.tail = TRUE) = %s",
          maxq,
          mean,
          sd,
          minq,
        ...
        pnorm_res <- sprintf(
          "pnorm(%s, %s, %s, lower.tail = TRUE) + pnorm(%s, %s, %s, lower.tail = FALSE) = %s",
          maxq,
          mean,
          sd,
          minq,
...
#' @param pages integer vector with pdf pages to extract
...
extract_pdf_pages <- function(file, pages, out = NULL) {
  ...
  msg <- sprintf(
    "Extracted from %s, from page %s to page %s!",
